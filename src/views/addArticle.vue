<template>
  <div class="add-article-container">
    ...
    <p>
      ...
      <input type="file" id="title" style="display: none;" ref="chooseFileRef" accept="*" @select="showSelectFile">
      <span v-if="!coverRef" class="add-icon" @click="chooseCover">
      ...
      <template v-else>
        <img :src="coverRef" alt="" class="cover">
        <svg t="1695872915310" class="delete-icon" viewBox="0 0 1024 1024" version="1.1"
          xmlns="http://www.w3.org/2000/svg" p-id="971" width="20" height="20" @click="coverRef = null">
          <path
    ...
    <p>
      ...
      <!-- <select id="tag-select" multiple>
        <option v-for="item in tagListRef" :key="item.id" :value="item.tag_name">{{ item.tag_name }}</option>
      </select> -->
    </p>
    <label for="content">文章内容</label>
    <Editor v-model="editorTxt" id="content" api-key="z9yxdow4c2vjkoulvv8b6dyg7ge1jete9gzqwpi5gj98sv6v" :init="{
...
function publishArticle() {
  if (!editorTxt.value) return;
  console.log(editorTxt.value)
  console.log('发布文章')
}
// 获取标签列表数据
